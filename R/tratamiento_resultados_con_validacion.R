...
setwd("D:/UDFnuevo")
getwd()
data_1cm <- read.table("text_summer1cm.txt",sep = " ", header = F)
data_05cm <- read.table("text_summer05cm.txt",sep = " ", header = F)
data_025cm <- read.table("text_summer025cm.txt",sep = " ", header = F)
data_no_pcm <-read.table("text-tfvalidacion.txt", sep= " ", header = F)

data_1cm_extract <- data_1cm %>%
  select(V1,V2,V3,V11)
data_05cm_extract <- data_05cm %>%
  select(V1,V2,V3,V11)
data_025cm_extract <- data_025cm %>%
  select(V1,V2,V3,V11)
...
data_produccion_filtrado[data_produccion_filtrado==0.0645307
] <- 1.72039e-02

date_1 <- as.Date('2022-04-09')
date_2 <- as.Date('2022-04-10')

time_1 <- c('09:00','9:30','10:00','10:30','11:00','11:30','12:00','12:30',
